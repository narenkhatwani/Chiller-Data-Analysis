...
def question_4() :
    ...
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))
    Label(screen4, text='Category to get highest maximum average rating: '+category_list[ratings_list.index(max_rating)], font=("Open Sans", 16, 'bold'), fg='black').pack() 
...
def display_inc_dec(year_count, year_list, first_year, second_year, third_year) :
    ...
    Label(screen6_1, text="Percentage installs increase from 2016 to 2017 : {}".format(((second_year-first_year)*100)/second_year), font=("Open Sans", 16, 'bold'), fg='black').pack() 
    Label(screen6_1, text="Percentage installs increase from 2017 to 2018 : {}".format(((third_year-second_year)*100)/third_year), font=("Open Sans", 16, 'bold'), fg='black').pack() 
    Label(screen6_1, text="Percentage installs increase from 2016 to 2018 : {}".format(((third_year-first_year)*100)/third_year), font=("Open Sans", 16, 'bold'), fg='black').pack() 
...
def question_9() :
    ...
    #d2=dataset_1[dataset_1.Rating>=4.1]
    avg_app_rating = d2.Rating.mean()
    #print('Average app rating = ', avg_app_rating)
    Label(screen9, text="Average App Rating : {}".format(avg_app_rating), font=("Open Sans", 16, 'bold')).pack()
    ...
    title('Average Ratings by Installs',size=20)
    totals = []
    for i in ax.patches:
...
def display_teen_mature_graph(required_contents, required_installs, teen_install, mature_install) :
    ...
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))
    Label(screen10_1, text="Ratio of downloads for the app that qualifies as teen versus mature17+: {} that is {}".format(Fraction(teen_install/mature_install).limit_denominator(), teen_install/mature_install), font=("Open Sans", 12, 'bold'), fg='black').pack() 
...
def display_year_quarters(result, year_list, quarters, install_count_list) :
    ...
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))
    Label(screen11_1, text= 'Result --> Year : '+str(result[2])+', Quarter : '+str(result[1])+', Installs : '+str(result[0]), font=("Open Sans", 16, 'bold'), fg='black').pack() 
...
def predict_subjectivity(reg) :
    if polarity_variable.get() == '':
        messagebox.showerror("Error", "Please provide some value for sentiment polarity!", parent=screen13_1)
        subjectivity_variable.delete(0, END)
    else :
        try :
            subjectivity_variable.delete(0, END)
            subjectivity = float(polarity_variable.get()) * reg.coef_[0][0] + reg.intercept_[0]
            subjectivity_variable.insert(END, subjectivity)
        except :
            messagebox.showerror("Error", "Please provide a proper value for sentiment polarity!", parent=screen13_1)
    
def show_relation() :
    global screen13_1, polarity_variable, subjectivity_variable
    screen13_1 = Tk()
    screen13_1.title('Question-13.1')
    adjustWindow(screen13_1)
    X = dataset_2['Sentiment_Polarity'].values.reshape(-1,1)
    y = dataset_2['Sentiment_Subjectivity'].values.reshape(-1,1)
    X_mean = nanmean(X, axis = 0)
    indices_X = where(isnan(X))
    X[indices_X] = take(X_mean, indices_X[1])
    y_mean = nanmean(y, axis = 0)
    indices_y = where(isnan(y))
    y[indices_y] = take(y_mean, indices_y[1])
    reg = LinearRegression()
    reg.fit(X,y)
    print('Linear Model is : Y = {:.5}X + {:.5}'.format(reg.coef_[0][0],reg.intercept_[0]))
    predictions = reg.predict(X)
    X2 = add_constant(X)
    est = OLS(y,X2)
    est2 = est.fit()
    f, ax = subplots(figsize=(9, 7))
    polarity_variable = StringVar()
    subjectivity_variable = StringVar()
    Label(screen13_1, text= 'Enter value for polarity(between -1 and 1)', fg='black').pack() 
    polarity_variable = Entry(screen13_1, width = 50)
    polarity_variable.pack()
    Button(screen13_1, text="Calculate Subjectivity", command= lambda: predict_subjectivity(reg)).pack()
    subjectivity_variable = Entry(screen13_1, width = 50)
    subjectivity_variable.pack()
    Label(screen13_1, text= 'Linear Model is : Y = {:.5}X + {:.5} (R-Squared value : {})'.format(reg.coef_[0][0],reg.intercept_[0], est2.rsquared), font=("Open Sans", 16, 'bold'), fg='black').pack() 
    scatter(dataset_2['Sentiment_Polarity'], dataset_2['Sentiment_Subjectivity'], c='black')
    plot(dataset_2['Sentiment_Polarity'],predictions,c='red',linewidth=3)
    xlabel('Polarity')
    ylabel('Subjectivity')
    show()    
...
def question_13() :
    ...
    Button(screen13, text="Relation", command= lambda: show_relation()).pack()
    ...
    for key, value in d.items(): 
        if key == 'Negative' :
            if value >= round(var/2) :
                Label(screen15, text="10 Best Foods for You - This app should not be launched! ", font=("Open Sans", 16, 'bold'), fg='black').pack()
                Label(screen15, text="Negative sentiments are greater than or equal to half of the positive sentiments ", font=("Open Sans", 16, 'bold'), fg='black').pack()
            else :
                Label(screen15, text="10 Best Foods for You - This app should be launched! ", font=("Open Sans", 16, 'bold'), fg='black').pack() 
                Label(screen15, text="Negative sentiments are not greater than or equal to half of the positive sentiments", font=("Open Sans", 16, 'bold'), fg='black').pack() 
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))
...
def question_16() :
    ...
    for year in true_year_list :
        ...
        a = 0
        sum_month = 0
        for mi in month_installs :
            sum_month += mi
            if mi == 0 :
                a += 1
        myNumber = sum_month / (12-a)
    ...
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))
    Label(screen16, text= 'Months that are the best indicator for average downloads over all years : '+dis_res[0]+' and '+dis_res[1], font=("Open Sans", 12, 'bold'), fg='black').pack() 
...
def question_17() :
    ...
    set(style="darkgrid")
    f, ax = subplots(figsize=(9, 7))   
    Label(screen17, text="There is a slight increase in installs with an increase in the size", font=("Open Sans", 16, 'bold'), fg='black').pack()     
                        ...
                        if android_version_variable.get() != '' and check(android_version_variable.get()) :
                            if category_variable.get() != '--Select Category--' and genre_variable.get() != '--Select Genre--' and install_variable.get() != '--Select No. of Installs--' and type_variable.get() != '--Select Type--'  and content_variable.get() != '--Select Content Rating--' and day_variable.get() != '--Select Day' and month_variable.get() != '--Select Month--' and year_variable.get() != '--Select Year--':
                                if type_variable.get() == 'Paid' and e.get() != '' and check(e.get()):
                                    ...
                                    with open('dummy_data_1.csv', 'a') as csvFile:
        ...
        if sentiment_option_variable.get() != '--Select Sentiment--': 
            if review_text.get("1.0",END) != '\n' :
                if check(sentiment_polarity_variable.get()) and check(sentiment_subjectivity_variable.get()):
                    ...
                    with open('dummy_data_2.csv', 'a') as csvFile:
                        writer_row = writer(csvFile)
                        writer_row.writerows(csv_data)
                    csvFile.close()
                    messagebox.showinfo("Success", "Data has been stored into the csv file!")
...
def data_wrangling() :
    ...
    dataset_1 = read_csv('Internship-project-Dataset1 (1).csv')  # First File
    dataset_2 = read_csv('C:\\Datasets\\Dataset2.csv')  # Second File  # Second File
...
def welcome_screen() :
    ...
    img = ImageTk.PhotoImage(Image.open("waving_android_background_by_the3dleopard_d4lgj2d.jpg"))
    panel = Label(welcome_screen, image = img)
    panel.pack(side = "bottom", fill = "both", expand = "yes")
